...
public class TrianglePair {
     ...
     */
    @ToStringExclude
    OrderBook baseMidPrice;
    /**
     * quote_cur/mid_cur p1
     */
    @ToStringExclude
    OrderBook quoteMidPrice;
     ...
     *
     *     '''
     * @param marketSellSize
     * @return
     */
    private Arbitrage negCycle(float marketSellSize) {
        float marketPriceBuy1 = this.marketPrice.getBids().get(0).getLimitPrice().floatValue();
    ...
    private float getMarketSellSize() {
        ...
        float marketSellSize = this.marketPrice.getBids().get(0).getRemainingAmount().floatValue() * order_ratio_base_quote;
        float baseMidBuySize = this.baseMidPrice.getAsks().get(0).getRemainingAmount().floatValue() * order_ratio_base_mid;
        ...
        size = (float) (Math.floor(Collections.min(ImmutableList.of(marketSellSize, baseMidBuySize)) * 10000) / 10000);
    ...
    private Arbitrage posCycle(float marketBuySize) {
        ...
        //TODO buy
        float baseMidPriceBuy1 = this.baseMidPrice.getBids().get(0).getLimitPrice().floatValue();
    ...
    private float getMarketBuySize() {
        ...
        float marketBuySize = this.marketPrice.getAsks().get(0).getRemainingAmount().floatValue() * this.order_ratio_base_quote;

        float baseMidSellSize = this.baseMidPrice.getBids().get(0).getRemainingAmount().floatValue() * this.order_ratio_base_mid;

        float baseQuoteOffReserveBuySize = this.accountAvailable(quoteCur, new CurrencyPair(baseCur, quoteCur)) - base_quote_quote_reserve;
        float quoteMidOffReserveBuySize = this.accountAvailable(midCur, new CurrencyPair(quoteCur, midCur)) - quote_mid_mid_reserve;
        float baseMidOffReserveSellSize = this.accountAvailable(baseCur, new CurrencyPair(baseCur, midCur)) - base_mid_base_reserve;
        ...
        size = (float) (Math.floor(Collections.min(ImmutableList.of(marketBuySize, baseMidSellSize)) * 10000) / 10000);
